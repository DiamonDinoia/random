...
class XoroshiroPlusPlus {
   public:
    inline constexpr explicit XoroshiroPlusPlus(
    ...
    inline constexpr XoroshiroPlusPlus(
        const XoroshiroPlusPlus& other) noexcept = default;
    inline constexpr XoroshiroPlusPlus(XoroshiroPlusPlus&& other) noexcept =
        default;
    inline constexpr XoroshiroPlusPlus& operator=(
        const XoroshiroPlusPlus& other) noexcept = default;
    inline constexpr XoroshiroPlusPlus& operator=(
        XoroshiroPlusPlus&& other) noexcept = default;
    ...
    constexpr void setState(std::array<std::uint64_t, 4> state) {
        m_state[0] = state[0];
        m_state[1] = state[1];
        m_state[2] = state[2];
        m_state[3] = state[3];
   ...
   private:
    ...
    //
    static inline constexpr std::uint64_t rotl(const std::uint64_t x, int k) {
        return (x << k) | (x >> (64 - k));
    }

    std::uint64_t next() {
        const std::uint64_t result =
            rotl(m_state[0] + m_state[3], 23) + m_state[0];
